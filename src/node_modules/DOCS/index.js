...
function wrap_isolated (handler, meta = {}, options = {}) {
  if (typeof handler === 'string') return wrap_legacy(handler, meta, options)
...
function wrap_legacy (handler_string, meta, options) {
  ...
  // eslint-disable-next-line no-new-func
  return new Function(params, source)(meta, create_sys_api)
}
...
function create_context (filename, sid) {
  ...
  function wrap_isolated_with_component (handler, doc, options) {
    const meta = { doc: resolve_handler_doc(handler, doc), sid, component: filename, resources }
    register_handler_doc(meta)
    return wrap_isolated(handler, meta, options)
  }
